...
(define (compile s
  ...
  (let ([compile (lambda (s) (compile s ns))])
    (cond
     [(pair? (syntax-e s))
...
(define (compile-lambda formals body ns)
  (define gen-formals
    (let loop ([formals formals])
      (cond
       ...
       [(pair? formals) (cons (loop (car formals))
                              (loop (cdr formals)))]
       [else null])))
  `(,gen-formals ,(compile body ns)))

(define (compile-let core-sym s ns)
